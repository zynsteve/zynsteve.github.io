...
<center>$$R[\alpha(x) = \alpha_i] = \sum_{j = 0}^{c-1}\lambda_{ij}P(Y = j| X = x)$$<br/><\center>
This is called the conditional risk given $X = x$

The overall risk is given by<br/>
<center>$$R = E[R(\alpha(X))] = \int_{x\in R^d} {R(\alpha(x))p(x)}dx$$<\center>
